...
namespace Dunjun
{
	...
	u8* Image::getPixel(u32 column, u32 row) const
	{
		...
		return pixels + (row * width + column) * (size_t)format; // get pixels info
	}
	void Image::setPixel(u32 column, u32 row, const u32* pixel)
	{
		...
		std::memcpy(p, pixel, (size_t)format); // does this have a use?
	}

	void Image::flipVertically()
	{
		std::size_t pitch = width * (size_t)format; // get the size of the info to be flipped
		...
		{
			u8* row = pixels + (i * width) * (size_t)format; // define row
			u8* oppositeRow = pixels + ((height - i - 1) * width) * (size_t)format;
